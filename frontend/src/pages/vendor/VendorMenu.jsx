...
export default function VendorMenu() {
  ...
  const openEdit = (item) => {
    setEditItem(item)
    setForm({ name: item.name, description: item.description || '', price_usd: item.price_usd, category: item.category || '' })
    setShowForm(true)
  }
  ...
  return (
    <div className="pb-24">
      <div className="flex items-center justify-between px-4 pt-14 pb-4">
        ...
        <button
          onClick={() => { setShowForm(true); setEditItem(null); setForm({ name: '', description: '', price_usd: '', category: '' }) }}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end">
          <div className="bg-white rounded-t-3xl w-full max-w-md mx-auto p-6 pb-10">
            ...
            {error && <div className="mb-3 p-3 bg-red-50 rounded-xl"><p className="text-sm text-red-600">{error}</p></div>}
