// Classic image upload — multer buffer/disk -> Cloudinary (or local dev disk).
//
// This controller serves POST /api/uploads (vendor logos, menu photos, delivery
// proof). It was originally upload.controller.js; the attachment feature later
// introduced a SESSION-based upload flow (createSession/putContent) and took over
// that filename, silently stranding this route. The two flows are different by
// design — this one is a single multipart POST returning a URL; sessions are for
// chat attachments with finalize semantics — so they live in separate files:
//   imageUpload.controller.js  -> upload.routes.js      (POST /api/uploads)
//   upload.controller.js       -> attachment.routes.js  (/api/attachments/uploads)
